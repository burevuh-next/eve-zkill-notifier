...
class EveBot(commands.Bot):
    def __init__(self):
        ...
        # Отключаем встроенную команду help, чтобы использовать свою
    ...
    async def setup_hook(self):
        ...
        all_cmds = [
            ping, check, status, add_to_watch, 
            remove_from_watch, init_channel, set_min_value, help_command
        ]
        for cmd in all_cmds:
    ...
    async def send_kill_notification(self, channel_id, killmail, event_type):
        ...
        sys_id = killmail.get('solar_system_id', 0)

        ids_to_fetch = [sys_id, ship_id, victim.get('character_id'), victim.get('corporation_id'),
                        attacker.get('character_id'), attacker.get('corporation_id')]
        ...
        if event_type == "PRIORITY_TARGET": color = discord.Color.gold()
